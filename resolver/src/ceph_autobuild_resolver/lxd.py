...
class LXDManager:
    ...
    def copy_from_snapshot(self, src: str, snapshot: str, dst: str) -> None:
        ...
        # ``start(wait=True)`` only waits for the container to reach RUNNING,
        # not for the system to finish booting.  Mirror ``launch`` and block
        # until boot completes — otherwise an immediate file push (e.g. the
        # validation diff) can race systemd's tmp setup and silently vanish.
        result = self.exec(dst, ["cloud-init", "status", "--wait"], check=False)
        if not result.ok:
            log.warning("cloud-init wait on %s reported %d", dst, result.returncode)
